...
import { BrandSubTitle, BrandTitle, ContainerUsuario, ContainerUsuarioText, ContainerVersao, LayoutSider, LinkSuporte, TextVersao, WrapperBrand } from './styles'
  ...
  return (
    ...
    >
        <WrapperBrand>
          ...
          {!collapsed && <BrandSubTitle>{process.env.REACT_APP_SUBNAME}</BrandSubTitle>}
          <ContainerVersao>
          ...
          {!collapsed && (
            <div>
              <LinkSuporte href="https://app.pipefy.com/public/form/5UPXYXx3" target="_blank" rel="noreferrer">
                Ajuda e Suporte
              </LinkSuporte>
            </div>
          )}
          <ContainerUsuario>
            <ContainerUsuarioText>
              {String(usuario?.nome || '').split(' ')?.[0] || ''} ({String(empresa.nome).trim()})
            </ContainerUsuarioText>
          </ContainerUsuario>
